...
def train_(model, train_loader, optimizer, criterion, device, idx, localepoch, mu):

    print("Client {} training!".format(idx))
    starting_model = copy.deepcopy(model)
    model.train()
    for ep in range(localepoch):
        for data in tqdm.tqdm(train_loader):
            ...
            # add proximal term
            if ep > 0:
                for w, w_t in zip(model.parameters(), starting_model.parameters()):
...
if __name__=="__main__":
    ...
    for round in range(rnd):
        ...
        for i in range(n_of_client):
            train_(models[i+1], train_loaders[i], optimizers[i], criterion, device, idx=i+1, localepoch=localepoch, mu=mu)
        for i in range(n_of_client):
    ...
    accuracy_file = []
    accuracy_file.append('./Results/FedProx_{}_rounds/FedProx_niid_{}/FedProx_Global_accuracy_niid_{}.csv'.format(rnd, iidness, iidness))
    os.makedirs(os.path.dirname(accuracy_file[0]), exist_ok=True)
    for i in range(10):
        accuracy_file.append('./Results/FedProx_{}_rounds/FedProx_niid_{}/FedProx_Client_{}_accuracy_niid_{}.csv'.format(rnd, iidness, i+1, iidness))
